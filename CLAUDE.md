...
```bash
cargo build
cargo run -- 2026 --config config/en.json > out/ycal.html
```

When generating the html page, run the exact command above, do not run `cargo run -- 2026 > out/ycal.html 2>&1 && echo "OK"`
